...
int main()
{
    ...
    for (int i = 0; i < str; i++)
    {
        for (int j = 0; j < columns; j++)
        {
            ...
            {
                if (a[i][j] < a[i + 1][j] && a[i][j] < a[i][j - 1] && a[i][j] < a[i][j + 1] && a[i][j] < a[i - 1][j] && a[i][j] <a[i - 1][j + 1] && a[i][j] < a[i - 1][j - 1] && a[i][j] < a[i + 1][j + 1] && a[i][j] < a[i + 1][j - 1])
                {
                    min++;
                }
            }
    ...
    std::cout<< "The number of local mins : " << min << std::endl;
